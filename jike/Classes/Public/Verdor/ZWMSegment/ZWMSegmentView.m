...
int const ZWMSegmentHeight = 50;//可根据项目需求设置高度
...
- (void)segmentPageSetting {
    ...
    for (int i = 0; i < _titles.count; i++) {
        ...
        if (i == 0) {
            ...
            // 添加指示杆
            _indicateView.frame = CGRectMake(_buttonSpace-50, _segmentHeight - _indicateHeight, titleSize.width, _indicateHeight);
            [_contentView addSubview:_indicateView];
            _indicateView.centerX = button.centerX;
        }
    }
...
- (void)setShowIndicateView:(BOOL)showIndicateView{
    ...
    _indicateView.hidden = showIndicateView;
